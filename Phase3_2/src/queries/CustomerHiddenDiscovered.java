...
public class CustomerHiddenDiscovered {
    ...
    private static final String MERGE_QUERY = "MERGE INTO CUSTOMER_HIDDEN_DISCOVERED_IN_GAME_SESSION CH " +
        "USING (SELECT %d AS GAME_SESSION_KEY, %d AS CUSTOMER_KEY, %d AS HINT_REVEALED_FLAG FROM DUAL) SOURCE " +
        "ON (CH.GAME_SESSION_KEY = SOURCE.GAME_SESSION_KEY AND CH.CUSTOMER_KEY = SOURCE.CUSTOMER_KEY) " +
        "WHEN MATCHED THEN UPDATE SET CH.HINT_REVEALED_FLAG = SOURCE.HINT_REVEALED_FLAG " +
        "WHEN NOT MATCHED THEN INSERT (GAME_SESSION_KEY, CUSTOMER_KEY, HINT_REVEALED_FLAG) " +
