...
public class TodaysEvent { // 기존 쿼리
    private static final String QUERY = "SELECT NC.NEW_DESCRIPTION, NC.AFFECTED_PRICE, NC.PLUS_MINUS, IC.CATEGORY_NAME, IC.CATEGORY_KEY, EN.AMOUNT, FROM NEWS_CATALOG NC JOIN EXISTING_NEWS EN ON NC.NCT_KEY = EN.NCAT_KEY JOIN ITEM_CATEGORY IC ON NC.CATEGORY_KEY = IC.CATEGORY_KEY WHERE EN.GAME_SESSION_KEY = ?";
            ...
            try (ResultSet queryResult = statement.executeQuery()) {
                ...
                do {
                    todaysEvent.add(new TodaysEvent(
                        ...
                        queryResult.getInt(3) * queryResult.getInt(6)
                    ));
